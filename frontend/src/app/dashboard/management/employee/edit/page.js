...
export default function EmployeeEditPage() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                await axios.put(`http://localhost:3001/employee/${employeeId}`, valuesToSend, { withCredentials: true });
                const redirectPath = userIdFromUrl ? `/dashboard/management/employee?companyId=${companyId}&userId=${userIdFromUrl}` : `/dashboard/management/employee?companyId=${companyId}`;
    ...
    useEffect(() => {
        if (!employeeId) {
            router.push(`/dashboard/management/employee?companyId=${companyId}`);
            return;
        }
        const fetchEmployee = async () => {
            try {
                const response = await axios.get(`http://localhost:3001/employee/${employeeId}`, { withCredentials: true });
